...
void
DledgerUtil::addRequestToLedger(shared_ptr<Ledger> dledger, const CaState& request) {
    //append / revoke record
    std::string identifier = (request.m_requestType == RequestType::REVOKE ?
                              std::string("Revoke_") : std::string("Cert_")) + readString(request.m_cert.getName().get(-1));
    Record record(RecordType::GENERIC_RECORD, identifier);
    if (request.m_requestType == RequestType::REVOKE) {
        record.addRecordItem(request.m_cert.getFullName().wireEncode());
    } else {
        record.addRecordItem(request.m_cert.wireEncode());
    }
